...
try {
    ...
    if (!$user) {
        $username = 'user_' . substr($sessionId, 6, 12);
        $stmt = $pdo->prepare("
            INSERT INTO users (username, email, session_id, civic_points, created_at)
            VALUES (?, '', ?, 0, NOW())
        ");
        ...
        // Also create user_devices entry so index.php can find this user
        $stmt = $pdo->prepare("
            INSERT INTO user_devices (user_id, device_session, is_active, created_at)
            VALUES (?, ?, 1, NOW())
        ");
