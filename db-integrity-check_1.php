...
if ($invalid > 0) {
    ...
    $bad = $pdo->query("SELECT eo.official_id, eo.full_name, eo.org_id, eo.title FROM elected_officials eo LEFT JOIN governing_organizations go ON eo.org_id = go.org_id WHERE go.org_id IS NULL LIMIT 20")->fetchAll(PDO::FETCH_ASSOC);
    foreach ($bad as $row) {
