 ...
 * 
 * Uses existing bot-detect.php and magic link email verification
 */
...
if (isset($_GET['token'])) {
    ...
    $stmt = $pdo->prepare("SELECT id, email FROM stands WHERE token = ? AND verified_at IS NULL AND token_expires > NOW()");
    $stmt->execute([$token]);
    $stand = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if ($stand) {
        $stmt = $pdo->prepare("UPDATE stands SET verified_at = NOW(), token = NULL, token_expires = NULL WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['email'])) {
    ...
    // Bot detection
    $formLoadTime = isset($_POST['_form_load_time']) ? (int)$_POST['_form_load_time'] : null;
    $botCheck = checkForBot($pdo, 'stand', $_POST, $formLoadTime);
    
    if ($botCheck['is_bot']) {
        // Silent rejection - pretend success
        ...
        if (!$email) {
            ...
            $stmt = $pdo->prepare("SELECT id, verified_at FROM stands WHERE email = ?");
            $stmt->execute([$email]);
            ...
            if ($existing && $existing['verified_at']) {
                ...
                if ($existing) {
                    ...
                    $stmt = $pdo->prepare("UPDATE stands SET token = ?, token_expires = ?, state_code = ? WHERE email = ?");
                    $stmt->execute([$token, $expires, $state, $email]);
                } else {
                    // Insert new
                    $stmt = $pdo->prepare("INSERT INTO stands (email, state_code, token, token_expires) VALUES (?, ?, ?, ?)");
                    $stmt->execute([$email, $state, $token, $expires]);
                }
                
                // Build magic link
                $baseUrl = "https://" . $_SERVER['HTTP_HOST'];
                $magicLink = "{$baseUrl}/0t/stand.php?token={$token}";
                
                // Send email
                $subject = "Verify Your Stand - The People's Branch";
                $body = "Click below to verify your stand and be counted:\n\n{$magicLink}\n\nThis link expires in 1 hour.\n\n---\nThe People's Branch\nNo Kings. Only Citizens.";
                $headers = [
